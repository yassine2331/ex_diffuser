...
from classifier.train_classifier_celeba import train_loop
...
def run():
    ...
    test_dataloader = get_dataloader(image_dir, attr_path, selected_attrs=selected_attrs, image_size=128, batch_size=batch_size, mode='test', num_workers=4)
    #--- Instantiate the model ---

    model = DiffusersLatentClassifier(input_channels=16, selected_attributes=selected_attrs).to(device)
    ...
    train_loop(config, model, noise_scheduler, optimizer, dataloader , test_dataloader=test_dataloader, lr_scheduler=lr_scheduler,model_name=model_name,vae=vae)
