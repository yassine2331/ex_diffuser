...
def CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , NUMBER_OFITERARIONS=1, weight=0.0):
    ...
    folder_name = folder_name+ "_final_test2_" +str(weight) + "/" + folder_name + "_"+ str(target_attrs[folder_name])+"_" + str(int(list(interventions.values())[0]))
    ...
    for i in tqdm(range(NUMBER_OFITERARIONS)):
        ...
        noise_end=202,
        intervention_strength=3,  # ⬅️ controls intensity of interventions
        interventions=interventions,
        w=weight,
        classifier=classifier
        )
...
def main():
    ...
    CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , 100,weight=5)
    ...
    CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , 100,weight=4)
    ...
    CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , 100,weight=3)
    ...
    CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , 100,weight=2)
    ...
    CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , 100,weight=1)
