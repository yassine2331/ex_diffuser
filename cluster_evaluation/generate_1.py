...
def CF_generation(model,vae,classifier, scheduler, source_loader, config, target_attrs, interventions , NUMBER_OFITERARIONS=1):
    ...
    folder_name = folder_name+ "_final_4" + "/" + folder_name + "_"+ str(target_attrs[folder_name])+"_" + str(int(list(interventions.values())[0]))
    ...
    for i in tqdm(range(NUMBER_OFITERARIONS)):
        ...
        noise_end=202,
        intervention_strength=5,  # ⬅️ controls intensity of interventions
        interventions=interventions,
        w=2.0,
        classifier=classifier
        )
